...
Page({ 
  ...
  data: {
    ...
    toName: "",
    toOpenId: "",
    nodata_str: "这篇文章还没有人点评，您要不要留下一笔～～～",
...
async commentreload(){
  ...
  onShareAppMessage: function() {
    var that = this;
    return {
      title: '推荐一篇文章：【萧梦语】' + that.data.post.title,
      path: '/pages/detail/detail?id=' + that.data.post._id,
      imageUrl: that.data.post.defaultImageUrl,
      success: (res) => {
  ...
  formSubmit: async function(e) {
    ...
    wx.showToast({
      title: '评论内容将由管理员审核后通过～～～',
      icon: 'none',
      duration: 3000,
      mask:true
    })
    // api.commentPush(e.detail.formId)
    ...
    app.checkUserInfo(async function(userInfo, isLogin) {
      if (!isLogin) {
        ...
        imageUrl = imageUrl.replace('http://', 'https://') 
          ...
          let qrCodeUrl = addReult.result[0].tempFileURL 
        var images = [{
            width: 62,
            height: 62,
            x: 32,
  ...
  popp: function() {
    ...
    var animationZhuan = wx.createAnimation({
      duration: 500,
      timingFunction: 'ease-out'
    })
    var animationShou = wx.createAnimation({
      duration: 500,
      timingFunction: 'ease-out'
    })
    var animationHai = wx.createAnimation({
      duration: 500,
      timingFunction: 'ease-out'
    })
    var animationDian = wx.createAnimation({
      duration: 500,
      timingFunction: 'ease-out'
    })
    var animationZan = wx.createAnimation({
      duration: 500,
      timingFunction: 'ease-out'
    })
    animationMain.rotateZ(720).step();
    animationDian.translate(0, -120 / 750 * systemInfo.windowWidth).rotateZ(720).opacity(1).step();
    animationShou.translate(0, -240 / 750 * systemInfo.windowWidth).rotateZ(720).opacity(1).step();
    animationHai.translate(0, -360 / 750 * systemInfo.windowWidth).rotateZ(720).opacity(1).step();
    animationZhuan.translate(0, -480 / 750 * systemInfo.windowWidth).rotateZ(720).opacity(1).step();
    animationZan.translate(0, -600 / 750 * systemInfo.windowWidth).rotateZ(720).opacity(1).step();
    this.setData({
      animMain: animationMain.export(),
      animDian: animationDian.export(),
      animZhuan: animationZhuan.export(),
      animShou: animationShou.export(),
      animHai: animationHai.export(),
      animZan: animationZan.export(),
    })
  },
  //收回动画
  takeback: function() {
    ...
    animationDian.translate(0, 0).rotateZ(0).opacity(0).step();
    animationShou.translate(0, 0).rotateZ(0).opacity(0).step();
    animationHai.translate(0, 0).rotateZ(0).opacity(0).step();
    animationZhuan.translate(0, 0).rotateZ(0).opacity(0).step();
    animationZan.translate(0, 0).rotateZ(0).opacity(0).step();
    this.setData({
      animMain: animationMain.export(),
      animDian: animationDian.export(),
      animZhuan: animationZhuan.export(),
